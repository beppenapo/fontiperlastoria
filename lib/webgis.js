...
/******************************************************************************/
function init() {
    layerList=["gsat","osm","comuni","toponomastica","aree_archeo","aree_architett","aree_archivi","aree_biblio","aree_cultmat","aree_cultmat_line","aree_foto","aree_orale","aree_stoart","aree_carto","highlightLayer","actLayer"];
    ...
    aree_cultmat_line = new OpenLayers.Layer.WMS("Aree cultura materiale linee", wmsHost
        ,{srs:srs3857,layers:'fps:aree_mater_line',format: format,transparent: true}
        ,{isBaseLayer: false,singleTile: true, ratio: 1,tileSize: tileSize}
    );
    aree_foto = new OpenLayers.Layer.WMS("Aree foto", wmsHost
        ,{srs:srs3857,layers:'fps:aree_foto_poly',format: format,transparent: true}
        ,{isBaseLayer: false,singleTile: true,ratio: 1,tileSize: tileSize}
    );
    aree_orale = new OpenLayers.Layer.WMS("Aree orale", wmsHost
        ,{srs:srs3857,layers:'fps:aree_orale_poly',format: format,transparent: true}
        ,{isBaseLayer: false,singleTile: true, ratio: 1,tileSize: tileSize}
    );
    aree_stoart = new OpenLayers.Layer.WMS("Aree storico-artistiche", wmsHost
        ,{srs:srs3857,layers:'fps:aree_stoart_poly',format: format,transparent: true}
        ,{isBaseLayer: false,singleTile: true, ratio: 1,tileSize: tileSize}
    );
    aree_carto = new OpenLayers.Layer.WMS("Aree cartografiche", wmsHost
        ,{srs:srs3857,layers:'fps:area_carto_poly',format: format,transparent: true}
    ...
    layerList.forEach(function(item, index){
        l=eval(item);
        map.addLayer(l);
        if (item.indexOf('aree_')>-1) {l.setVisibility(false);}
    });
...
ubi_stoart.setVisibility(false);
    ...
    listalayer= [aree_carto, aree_archeo, aree_archivi, aree_architett, aree_biblio, aree_cultmat,aree_cultmat_line, aree_foto, aree_orale, aree_stoart,aree_carto];
